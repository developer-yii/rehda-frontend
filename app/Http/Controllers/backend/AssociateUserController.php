...
{
    public function associateUsers(Request $request)
    {
        $user = Auth::user();
        ...
        return view('backend.members.users.associate', compact('companies'));
    }

    private function exportToCsv($query)
    {
        $filename = 'ListingAssociateUsers' . now()->format('YmdHis') . 'Export.csv';
        $headers = [
