...
class SubsidiaryUserController extends Controller
{
    public function subsidiaryUsers(Request $request)
    {
        $user = Auth::user();
        ...
        return view('backend.members.users.subsidiary', compact('companies'));
    }

    private function exportToCsv($query)
    {
        $filename = 'ListingSubsidiaryUsers' . now()->format('YmdHis') . 'Export.csv';
        $headers = [
