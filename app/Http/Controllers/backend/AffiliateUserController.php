...
{
    public function affiliateUsers(Request $request)
    {
        $user = Auth::user();
        ...
        return view('backend.members.users.affiliate', compact('companies'));
    }

    private function exportToCsv($query)
    {
        $filename = 'ListingAffiliateUsers' . now()->format('YmdHis') . 'Export.csv';
        $headers = [
