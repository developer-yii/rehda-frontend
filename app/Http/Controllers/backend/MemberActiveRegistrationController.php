...
class MemberActiveRegistrationController extends Controller
{
    ...
    public function activeMembers(Request $request)
    {
        ...
        return view('backend.members.active', compact('branches'));
    }
    ...
    public function profileEdit(Request $request, $mid)
    {
        // Check if the user is authorized to edit members
        if (!Auth::user()->can('activememberprofile-edit')) {
            return redirect()->back()->with('error', 'You have not permission to Edit members profile.');
        }
        ...
        if ($mid) {
            ...
            return view('backend.members.edit-member-profile', compact('mid', 'salutations', 'states', 'genders', 'countries', 'plantiers', 'ordinaryMembers', 'data', 'userProfile'));
        }
    }

    public function profileUpdate(MemberProfileRequest $request, $id)
    {
        if ($request->ajax() && $id) {
            $data = $this->memberService->updateActiveMemberProfile($request, $id);
            return response()->json($data);
        }
