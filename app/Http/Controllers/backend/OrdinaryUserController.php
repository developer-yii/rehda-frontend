...
{
    public function ordinaryUsers(Request $request)
    {
        $user = Auth::user();
        ...
        return view('backend.members.users.ordinary', compact('companies'));
    }

    private function exportToCsv($query)
    {
        $filename = 'ListingOrdinaryUsers' . now()->format('YmdHis') . 'Export.csv';
        $headers = [
